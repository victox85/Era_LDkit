import { QueryEngine , createLens, type Options, createNamespace } from "ldkit";
import { dc, rdfs, xsd, owl, rdf } from "ldkit/namespaces";
import { CONSTRUCT } from "ldkit/sparql";
import { ldkit } from "ldkit/namespaces";
import { DataFactory } from "ldkit/rdf";
/**
 * European Union Raylway Core Metadata Element Set, Version 1.1
 *
 * `@dc: <http://data.europa.eu/949/>`,
 */
...
export const TunnelSchema = {
  "@type": era.Tunnel,          // 👈 importante
  label: rdfs.label,
  inCountry: era.inCountry,
  length: {
    "@id": era.lengthOfTunnel,
    "@type": xsd.decimal,       // 👈 aquí sí se permite @type
  },
} as const;
...
for (const t of results) {
  ...
  for (const r of tunnels) {
    console.log({
      p: r.p?.value,
      inCountry: r.inCountry?.value,
      numTotalTracks: r.numTotalTracks?.value,
      numTracksWithPropertyAsCoreParameter: r.numTracksWithPropertyAsCoreParameter?.value,
      numTracksWithoutPropertyAsCoreParameter: r.numTracksWithoutPropertyAsCoreParameter?.value,
      numTracksWithoutTrainDetectionSystem: r.numTracksWithoutTrainDetectionSystem?.value,
    });
  }
}*/
